...
(Test alert by evaluating: =(alert "test")=. Did you see a notification? If so, proceed. If not, you'll need to figure out what went wrong. For me, ='notifications= will show a notification but does not obey all options (e.g., =alert-fade-fime=). ='libnotify= works much better. You'll have to fend for yourself on this front.)
