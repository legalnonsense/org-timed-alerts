...
(Test =alert= by evaluating: =(alert "test")=. Did you see a notification? If so, proceed. If not, figure out what went wrong. Note: for me, setting =alert-default-style= to =notifications= will show a notification but does not obey all alert options (e.g., =alert-fade-fime=). Setting the default style to =libnotify= works much better. Obviously this is OS dependent, so you're on your own.)
